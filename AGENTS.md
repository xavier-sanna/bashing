...
This repository is a small Bash helper library rooted in `src/`. `src/load.sh` is the main entrypoint, `src/bootstrap.sh` resolves task and project paths, and `src/self-update.sh` updates installed copies. General helpers live in `src/utils/`, Docker helpers in `src/docker/compose.sh`, UI modules under `src/ui/`, and privilege helpers in `src/privileges/`. Example consumer scripts live in `examples/`.

## Build, Test, and Development Commands
There is no build step. Use lightweight Bash validation instead:

- `bash -n install.sh src/*.sh src/utils/*.sh src/docker/*.sh src/ui/*.sh src/privileges/*.sh examples/* tests/test_helper/*.bash`: syntax-check shell scripts, examples, and shared test helpers.
...
## Testing Guidelines
Tests use Bats and live in `tests/`. Add new tests as `tests/<feature>.bats`; put shared helpers in `tests/test_helper/*.bash`. For each change, run `bash -n install.sh src/*.sh src/utils/*.sh src/docker/*.sh src/ui/*.sh src/privileges/*.sh examples/* tests/test_helper/*.bash`, `shellcheck ...` when available, and `mise run test`.
